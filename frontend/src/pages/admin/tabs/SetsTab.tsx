...
export default function SetsTab() {
  ...
  const load = async () => {
    try {
      const data = await api.admin.getSets();
      setSets(data);
    } catch {
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    setError("");
    try {
      await api.admin.createSet(name);
      setNewName("");
      await load();
    } catch (err: any) {
  ...
  const handleUpdate = async (id: number) => {
    ...
    setError("");
    try {
      await api.admin.updateSet(id, name);
      setEditId(null);
      await load();
    } catch (err: any) {
  ...
  const handleDelete = async (id: number, name: string) => {
    if (!confirm(`Delete set "${name}"? Questions and students assigned to this set will be affected.`)) return;
    setError("");
    try {
      await api.admin.deleteSet(id);
      await load();
    } catch (err: any) {
  ...
  return (
    <div className="max-w-2xl">
      <div className="mb-6">
        <h2 className="text-lg font-bold text-stone-900">Paper Sets</h2>
        <p className="text-sm text-stone-500 mt-1">
          Manage exam paper sets (e.g., A, B, C). Students are assigned to a set to determine which paper they receive.
        </p>
      </div>
      ...
      <form onSubmit={handleAdd} className="flex gap-3 mb-6">
        <input
          ...
          placeholder="New set name (e.g. C)"
      ...
      <div className="bg-white border border-stone-200 rounded-lg overflow-hidden shadow-sm">
        {loading ? (
          ...
          <div className="text-center py-12 text-stone-400 text-sm">No sets defined</div>
        ) : (
                      ...
                      {editId === s.id ? (
                        <>
                          <button
                            ...
                            className="p-1.5 text-teal-600 hover:bg-teal-50 rounded transition-colors shadow-sm"
                          >
